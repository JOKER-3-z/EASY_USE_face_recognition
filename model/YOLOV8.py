...
class YOLOv8_face:
    ...
    def draw_detections_with_nopoint(self, image, boxes,names):
        for box,name in zip(boxes,names):
            x, y, w, h = box.astype(int)
            # Draw rectangle
            cv2.rectangle(image, (x, y), (x + w, y + h), (0, 0, 255), thickness=1)
            cv2.putText(image, name+" ", (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255),
                        thickness=2)
        return image

    def crop_detections(self,image,boxes):
        imagelist=[]
        for box in boxes:
            x, y, w, h = box.astype(int)
